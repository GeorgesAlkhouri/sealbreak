...
flowchart TB
    ...
    subgraph PHONE["iPhone"]
        subgraph APP["Sealbreak app process and sandbox"]
            ...
            SETUPSTATE[("Setup lifecycle state")]
...
Replacement updates the existing Keychain item rather than deleting it first. Initial setup authenticates with Face ID before persistent mutation, then writes an explicit `pending(profileID)` setup state before creating the profile and profile-scoped Keychain item. Setup becomes usable only after an explicit `ready(profileID)` commit. Profile data without a matching ready state, pending state, mismatched state, unreadable state, or multiple profiles is treated as recovery-required and never as a valid configured app. Setup does not attempt rollback from a cancelled feature effect. Instead, any interrupted persistence after the pending marker remains fail-closed until the user performs the confirmed full local reset. Removing a profile also removes its matching setup lifecycle state. All readers and writers use a common encoded-record size limit, and an oversized replacement is rejected before the Keychain update. A full reset deletes all Keychain items in Sealbreak's unseal service namespace before removing the profile catalog and setup lifecycle state. Replacement and reset still cannot prove that an independent recovery copy exists or is usable.
...
| **M08 — Safe share lifecycle** | Application | Require fresh authorization for sensitive operations, require explicit pending/ready setup lifecycle state, reject uncommitted or mismatched local state, use safe in-place updates, and enforce one encoded storage-size invariant across readers and writers |
