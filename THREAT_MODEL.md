...
Replacement updates the existing Keychain item rather than deleting it first. Initial setup authenticates with Face ID before persistent mutation, then creates the profile catalog entry directly in a `pending` lifecycle state before creating the profile-scoped Keychain item. Setup becomes usable only after that same catalog entry is atomically rewritten as `ready`. A pending entry, unreadable catalog, unsupported catalog version, or multiple profiles is treated as recovery-required and never as a valid configured app. Version-1 profile catalogs are deliberately not migrated. Setup does not attempt rollback from a cancelled feature effect. Instead, any interrupted persistence after the pending entry remains fail-closed until the user performs the confirmed full local reset. All readers and writers use a common encoded-record size limit, and an oversized replacement is rejected before the Keychain update. A full reset deletes all Keychain items in Sealbreak's unseal service namespace before removing the profile catalog. Replacement and reset still cannot prove that an independent recovery copy exists or is usable.
...
| **M08 — Safe share lifecycle** | Application | Require fresh authorization for sensitive operations, require explicit pending/ready lifecycle state in the profile catalog, reject pending or unsupported local state, use safe in-place updates, and enforce one encoded storage-size invariant across readers and writers |
